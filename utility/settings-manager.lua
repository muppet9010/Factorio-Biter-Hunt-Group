...
Settings.HandleSettingWithArrayOfValues = function(settingType, settingName, expectedValueType, defaultValue, globalGroupsContainer, globalSettingContainerName, globalSettingName, valueHandlingFunction)
    ...
    if tableOfValues ~= nil and type(tableOfValues) == "table" then
        for id, value in pairs(tableOfValues) do
            local globalSettingContainer = Settings.CreateGlobalGroupSettingsContainer(globalGroupsContainer, id, globalSettingContainerName)
            local typedValue = ValueToType(value, expectedValueType)
            if typedValue ~= nil then
                globalSettingContainer[globalSettingName] = valueHandlingFunction(typedValue)
            else
                globalSettingContainer[globalSettingName] = valueHandlingFunction(defaultValue)
        ...
        local globalSettingContainer = Settings.CreateGlobalGroupSettingsContainer(globalGroupsContainer, 0, globalSettingContainerName)
        globalSettingContainer[globalSettingName] = valueHandlingFunction(defaultValue)
    else
        local globalSettingContainer = Settings.CreateGlobalGroupSettingsContainer(globalGroupsContainer, 0, globalSettingContainerName)
